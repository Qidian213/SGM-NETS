...
class KittiDataset():
    def __init__(self, train_file, mode='train', n_samples=None):
        ...
        self.d_cost = np.memmap('/home/zzg/Datasets/kitti/leftbin/000002_10.bin', dtype=np.float32, shape=(1, 70, 375, 1242))
        p = np.isnan(self.d_cost)
        self.d_cost[p] = 0.0  
        self.d_cost = torch.tensor(self.d_cost)
        self.d_cost = self.d_cost.squeeze(0)
        self.d_cost = self.d_cost.type(torch.FloatTensor)  # [70,h,w]
    ...
    def getitem(self, i):
       # image_name = self.image_lists[i]
        ...
        while(disp_val==0 or disp_val>69):
            x = random.randint(300,img_w-200)
            y = random.randint(100, img_h-100)
...
#        d_cost_d = d_cost[:,y+1:img_h-3,x]
        d_cost_l = self.d_cost[:,y,120:x+1]
        d_cost_r = self.d_cost[:,y,x:img_w-4]
        d_cost_u = self.d_cost[:,3:y+1,x]
        d_cost_d = self.d_cost[:,y:img_h-4,x]
