...
from dataload_new import KittiDataset
from SGMNet import SGM_NET
from sgm_pathloss import pathloss

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
writer = SummaryWriter("/home/wsy/work/SGM_MACHING/sgm_nets2/work_sapce")

model = SGM_NET()
#model.load_state_dict(torch.load('/home/wsy/work/SGM_MACHING/sgm_nets2/work_sapce/SGMNet.pkl'))
...
train_loader = KittiDataset('/home/wsy/datasets/training/train.txt')
...
for epoch in range(10):
    ...
    for iter_num in range(int(train_len/batch_size-1)):
        ...
        for bnum in range(batch_size):
            image_patchs_l, patch_cods_l, image_patchs_r, patch_cods_r, image_patchs_u, patch_cods_u, image_patchs_d, patch_cods_d, origin_patch, origin_cod ,disp_val, d_cost_l , d_cost_r, d_cost_u, d_cost_d, d_cost_x0 ,x,y= train_loader.getitem(trained_num + bnum)
...
#            print(image_patchs_d.size(), patch_cods_d.size())
    ...
    #    print(grad_all.size())
        ...
        iter_num_all = iter_num_all+1
        print(p1p2_x0s)
        print('Iter_num: {}\t Loss: {}\t Grad: {}'.format(iter_num_all,loss_sum,grad_all))
        writer.add_scalar('loss',loss_sum,iter_num_all)
        if iter_num%10 == 0:
